...
> 来源: https://res.anjian.com/expand//Find.html
...
result = iOSElement.Find(selector)
If result["code"] = 0 Then 
    Dim sections_count, rows
    sections_count = result["data"]["numberOfSections"]
    TracePrint "当前UITableView控件共有：" & sections_count & "组"
    rows = Split(result["data"]["rows"], ",")
    For i = 1 To Len(rows)
        TracePrint "第" & i & "组有" & rows[i] & "个子项"
    Next
Else 
    TracePrint result["msg"]
End If
