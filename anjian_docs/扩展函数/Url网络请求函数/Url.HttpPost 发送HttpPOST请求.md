...
> 来源: https://res.anjian.com/expand/url/HttpPost.html
...
| data | string | 否 | 需要提交的数据，包含中文或空格等符号时， 建议URL编码后传入 |
...
| file | string | 否 | 要上传的文件路径（目前仅iOS新版v200以上支持） |
...
```sourceCode lua
Dim ret,body = {null}
body["url"] = "https://httpbin.org/post?page=1"
body["header"] = {"Authorization":"Bearer my_secret_toke"}
body["data"] = "user_id=12345&token=abcdef"
ret = Url.HttpPost(body)
TracePrint ret
```
