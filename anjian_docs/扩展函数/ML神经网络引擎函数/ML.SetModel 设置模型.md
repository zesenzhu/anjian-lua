...
> 来源: https://res.anjian.com/expand/ml/SetModel.html
...
ret = ml.setModel(index,modelPath\[,confidenceThreshold,bid,options\])
...
| 参数名 | 参数类型 | 是否必选 | 参数说明 |
| --- | --- | --- | --- |
| index | int | 是 | 模型索引，建议取值范围：0-9 |
| modelPath | string | 是 | 模型本地目录路径 |
| confidenceThreshold | int | 否 | 最低可信度，低于该可信度的目标数据将不返回，取值范围：0.1-1.0，默认值：0.25 |
| bid | int | 否 | 目标应用包名，一般情况下留空：“” |
