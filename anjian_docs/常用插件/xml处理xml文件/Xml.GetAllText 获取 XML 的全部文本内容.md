# Xml.GetAllText 获取 XML 的全部文本内容

> 来源: https://res.anjian.com/plugin/xml/GetAllText.html
...
递归遍历指定的 XML 节点及其所有子节点，提取其中的**全部文本内容**，并按指定分隔符拼接后返回字符串结果
...
text = Xml.GetAllText(xmlData\[, sep\])
...
| sep | string | 否 | 文本拼接时使用的分隔符；不传时默认使用空字符串 |

### 函数返回

| 返回名 | 返回类型 | 返回说明 |
| --- | --- | --- |
| text | string | 返回 XML 节点及其子节点中所有文本拼接后的结果字符串 |
...
If xmlNode Then 
    TracePrint "找到的节点的文本为：" & Xml.GetAllText(xmlNode, "\n")
End If
```
